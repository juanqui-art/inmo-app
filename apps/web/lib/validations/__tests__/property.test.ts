...
describe('createPropertySchema', () => {
  describe('price validation', () => {
    it('should reject negative prices', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('El precio debe ser mayor a 0')
      }
    })
    ...
    it('should reject extremely high prices', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('El precio es demasiado alto')
      }
    })
  })
  ...
  describe('title validation', () => {
    it('should reject titles that are too short', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('El título debe tener al menos 5 caracteres')
      }
    })
    ...
    it('should reject titles that are too long', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('El título no puede exceder 100 caracteres')
      }
    })
  ...
  describe('description validation', () => {
    it('should reject descriptions that are too short', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('La descripción debe tener al menos 20 caracteres')
      }
    })

    it('should accept valid descriptions', () => {
      const description = 'Una hermosa casa con amplio jardín y excelente ubicación en zona residencial'
      const result = createPropertySchema.safeParse({
        title: 'Casa en venta',
        description,
        price: 250000,
  ...
  describe('complete valid property', () => {
    it('should accept a fully populated valid property', () => {
      const result = createPropertySchema.safeParse({
        ...
        description: 'Una casa de 3 habitaciones y 2 baños con amplio jardín, garaje para 2 autos y excelente ubicación cerca de escuelas y centros comerciales',
        price: 350000,
        transactionType: 'SALE',
        category: 'HOUSE',
        status: 'AVAILABLE',
      ...
      expect(result.success).toBe(true)
      if (result.success) {
        expect(result.data.title).toBe('Hermosa casa con jardín en zona residencial')
...
describe('updatePropertySchema', () => {
  it('should require a valid UUID', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toContain('ID de propiedad inválido')
    }
  })
